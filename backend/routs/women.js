...
router.get('/', async (req, res) => {
  try {
    products = await Product.find({ gender:"female", quantity: { $gt: 0 } });
...
router.post('/', async (req, res) => {
  try {
    ...
    let products;
    if (sortBy === 'name') {
      products = await Product.find({ gender: "female", quantity: { $gt: 0 } }).sort({ name: 1 });
    } else if (sortBy === 'price') {
      products = await Product.find({ gender: "female", quantity: { $gt: 0 } }).sort({ price: 1 });
    } else {
      // Handle invalid sorting option
      return res.status(400).send('Invalid sorting option');
    }

    res.render('women', { products }); // Corrected template name
  } catch (error) {
    console.error(error);
    res.status(500).send('Internal Server Error');
  }
 ...
 router.post('/Addto', (req, res) => {
  ...
  if (action) {
    ...
    if (actionType === 'cart') {
      ...
      res.redirect('/women'); // Redirect back to the men page
